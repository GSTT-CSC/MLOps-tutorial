...
def plot_inference_test(net, dm, n_samples_plot=4):
    ...
    with torch.no_grad():
        ...
        if n_samples_total == 0:
            ...
            return
        fig, axs = plt.subplots(len(sample_idx), 4, figsize=(20, 20), dpi=80)
        ...
        for i, n in enumerate(sample_idx):
            ...
            pred_image = torch.argmax(val_outputs, dim=1).detach().cpu()[0, :, :, test_slice]

            axs[i, 0].set_title(f"image {k}")
            axs[i, 0].imshow(input_image, cmap="gray")
            axs[i, 1].set_title(f"LABEL_TRUE")
            axs[i, 1].imshow(label_image)
            axs[i, 2].set_title(f"LABEL_PRED")
            axs[i, 2].imshow(pred_image)
            axs[i, 3].set_title(f"LABEL_PRED_DIFF {k}")
