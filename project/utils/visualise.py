...
def plot_inference_test(net, dm, n_samples_plot=4):
    net.eval()
    device = (
        torch.device("cuda:0") if torch.cuda.is_available() else torch.device("cpu:0")
    )
    net.to(device)
    ...
    with torch.no_grad():
        ...
        sample_idx = random.sample(
            range(n_samples_total), min(n_samples_total, n_samples_plot)
        )
        ...
        fig, axs = plt.subplots(
            len(sample_idx), 4, figsize=(int(5 * len(sample_idx)), 20), dpi=80
        )
        ...
        for i, n in enumerate(sample_idx):
            ...
            pred_image = np.array(
                torch.argmax(val_outputs, dim=1).detach().cpu()[0, :, :, test_slice]
            )
