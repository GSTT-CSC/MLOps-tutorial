...
class LoadImageXNATd(MapTransform):
    ...
    def __init__(
        ...
        image_loader: Transform = LoadImage(),
        ...
        for key in self.keys:
            ...
            if key in data:
                ...
                if "localhost" in adjusted_xnat_config.get("server", ""):
                    adjusted_xnat_config["server"] = adjusted_xnat_config[
                        "server"
                ...
                with xnat.connect(
                    ...
                    if len(set([x["data_label"] for x in d[key]])) != len(
                        [x["data_label"] for x in d[key]]
                    ):
                        ...
                        while attempts < 3:
                            try:
                                with tempfile.TemporaryDirectory() as tmpdirname:
                                    session_obj = session.create_object(
                                        item["action_data"]
                                    )
                                    session_obj.download_dir(
                                        tmpdirname, verbose=self.verbose
                                    )
                                    ...
                                    # image loader needs directory path to load 3D images
                                        ...
                                        if len(image_dirs) > 1:
                                            if self.image_series_option == "error":
                                                ...
                                                self.image_series_option == "keepfirst"
                                            ):
                                                ...
                                                raise ValueError(
                                                    f"More than one image series found in {images_path} and no option specified"
                                                )
                                    break
                                ...
                                if attempts == 3:
                                    raise Exception(
                                        f"Image loader failed on {item} after 3 retries due to {e}"
                                    )
